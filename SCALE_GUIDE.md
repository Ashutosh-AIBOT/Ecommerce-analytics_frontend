...
At each stage this file answers one question:
**What do I change in the frontend when traffic grows?**
...
Images    → raw static files
```

Nothing to optimize yet. Focus is making it work.
...
Bundle    → vite build (auto minified)
Images    → compress before pushing
```

**What to add:**
- Compress all images before uploading
- Make sure `npm run build` output is under 500KB
...
- Split Dashboard page from Home page bundle
- Convert images to WebP format
- Add loading spinner for churn form
...
Users     → 50K → 500K
CDN       → Cloudflare Pro
Bundle    → tree shaking reviewed
Caching   → cache static assets 1 year
```
...
```
Users     → 500K+
CDN       → Akamai or custom CDN
Bundle    → micro frontends if needed
```

**What to add:**
- Split into micro frontends per page
- Dedicated CDN per region
...
| [RUNBOOK.md](./RUNBOOK.md) | How to build and deploy |


--------------------------------------------
###  That Common formate that i should Follow as my current learnings.
